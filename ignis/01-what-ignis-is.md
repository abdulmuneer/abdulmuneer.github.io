...
- **`custom_backend`** *(backend)* - Not an agent but a drop-in engine: it runs the whole harness against any OpenAI-compatible endpoint (`max serve`, Ollama, OpenAI). `cached_tokens` is always 0 there, because the REST boundary hides `num_cached_tokens` - which is exactly why in-process stays the only path to real cache telemetry.
