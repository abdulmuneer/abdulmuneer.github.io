...
| **M4** Mojo in the graph | ✅ done (with a ceiling) | A Mojo custom op compiled into the MAX graph and run inside the live sampler; an airtight grammar gate via MAX's own engine. See [Part 4](./04-max-the-platform.md). |
...
## Three more changes after the milestones

M5 was meant to be the end of it. But once the harness actually worked, three questions remained. Each one needed the feature built and then measured. Here is how they landed.
...
- **The confirmation signal is a coarse keyword check**, but the order-id binding underneath is what actually enforces the gate. But these are examples to be built upon in future.
...
Read against those caveats, the result is narrow but solid: a compiled, deterministic, in-process harness with real cache telemetry, byte-exact tool replay, a policy enforced inside the model's decode loop, a second in-process pipeline doing real retrieval, and a conversation that warm-restarts across processes - all with a model-free CI path behind it. It isn't the drop-in dream [Part 0](./00-curiosity-and-the-plan.md) started from. It's a smaller thing I can stand behind.
