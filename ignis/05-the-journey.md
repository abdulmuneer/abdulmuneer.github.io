...
**Three changes after I thought I was done.** In-process RAG came first. The
open question was whether a second MAX pipeline would collide with the first
over that same process-global context. It doesn't:
`task=PipelineTask.EMBEDDINGS_GENERATION` loads an embedding model beside the
LLM, and a Mojo SIMD dot-product kernel ranks the in-memory corpus.

The next pass turned the small retrieval fixture into a program I could use on
a document tree. `rag_search` now persists a SQLite FTS5 index, refreshes full
or selected scopes in one transaction, and returns line citations with source
and snapshot identity. It has a CLI, Python API, read-only Mojo bridge, and an
authenticated loopback HTTP adapter. The work was mostly operational: reject
symlink components, bound every input and queue, preserve indentation, keep
failed refreshes from changing the previous snapshot, redact request content,
coordinate integrity checks with WAL writers, and document backup and rebuild.
Python owns those filesystem, database, and HTTP facilities. Mojo owns the
compiled bridge contract. That result changed the question from “how much can I
port?” to “which side should own this interface?”

Speculative decoding was the third change. Three MAX bugs stood between the
config flag and a running draft/target pipeline; the benchmark then showed a
slowdown for this pair: 141.6 tok/s against a 165.7 baseline, with about 5.5
ms/cycle of host overhead. It remains opt-in and off by default for the tested
8B target.
...
The repository therefore contains both kinds of boundary: compiled Mojo in the
decode loop and semantic ranking path, and a Mojo-to-Python bridge for durable
local search. The code and tests identify which runtime owns each operation.
